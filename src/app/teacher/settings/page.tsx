...
export default function SettingsPage() {
  ...
  // TAMBAHAN: State untuk melihat password siswa tertentu
              ...
              <div className="pt-2 border-t border-slate-100 space-y-2">
                ...
                {students.length === 0 ? (
                  ...
                  <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                    {students.map((s, idx) => (
                      <div key={s.student_id} className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex flex-col gap-2 text-xs">
                        {editingStudentId === s.student_id ? (
                          ...
                          <>
                            ...
                            {/* Baris Informasi Password Default Siswa */}
                            <div className="flex items-center justify-between bg-white px-2.5 py-1.5 rounded-lg border border-slate-200 text-[11px]">
                              <div className="flex items-center gap-1.5">
                                <span className="text-slate-400 font-medium">Password Default:</span>
                                <code className="font-mono font-bold text-slate-700 bg-slate-100 px-1.5 py-0.5 rounded">
                                  {visiblePasswords[s.student_id] ? (s as any).password || 'Tidak ada data' : '••••••••'}
                                </code>
                              </div>
                              <button
                                onClick={() => {
                                ...
                                className="text-[10px] font-bold text-slate-600 hover:text-blue-600 bg-slate-100 hover:bg-blue-50 px-2 py-0.5 rounded transition"
                              >
                                {visiblePasswords[s.student_id] ? 'Sembunyikan' : 'Lihat'}
                              </button>
                            </div>
                          </>
                        )}
