...
export default function WordSearchEngine({ data, mode = 'challenge', onComplete }: WordSearchEngineProps) {
  ...
  if (!data || !data.words || data.words.length === 0) {
    return <div className="p-8 text-center text-sm text-slate-400">Konfigurasi Word Search kosong.</div>;
  }

  const normalizedWords: TargetWordItem[] = data.words.map((item) => {
    if (typeof item === 'string') {
      return { word: item.replace(/\s+/g, '').toUpperCase(), hint: '' };
    }
    return {
      word: (item.word || '').replace(/\s+/g, '').toUpperCase(),
      hint: item.hint || '',
    };
  });

  const targetWordsStrings = normalizedWords.map((w) => w.word);
  const maxLength = Math.max(...targetWordsStrings.map((w) => w.length));
  const gridSize = Math.max(10, maxLength + 3);

  const { grid, placedWords: initialPlacedWords } = useMemo(() => {
    ...
    normalizedWords.forEach((item) => {
      ...
      while (!placed && attempts < 100) {
        ...
        const maxR = gridSize - (dr === 1 ? word.length : 0);
        const maxC = gridSize - (dc === 1 ? word.length : 0);
    ...
    return { grid: matrix, placedWords: placements };
  }, [data, gridSize]);

  const [activeWords, setActiveWords] = useState<PlacedWord[]>(initialPlacedWords);

  const foundCellsMap = useMemo(() => {
    const map = Array.from({ length: gridSize }, () => Array(gridSize).fill(false));
    activeWords.forEach((item) => {
      if (!item.found) return;
      const { row, col, direction, word } = item;
    ...
    return map;
  }, [activeWords, gridSize]);

  // Cek apakah semua kata sudah ditemukan & Trigger Confetti 🎉
  useEffect(() => {
    if (activeWords.length > 0 && activeWords.every((w) => w.found)) {
      playSuccessSound();
  ...
  const handleVerifySelection = () => {
    ...
    const formedWord = selectedCells.map((cell) => grid[cell.r][cell.c]).join('');
    const reversedFormedWord = [...selectedCells].reverse().map((cell) => grid[cell.r][cell.c]).join('');

    const matchedIndex = activeWords.findIndex(
      (item) => !item.found && (item.word === formedWord || item.word === reversedFormedWord)
    );
    ...
    if (matchedIndex !== -1) {
      ...
      setScore((prev) => prev + Math.round(100 / targetWordsStrings.length));
    ...
    <div className="w-full p-4 md:p-8 rounded-[2.5rem] shadow-2xl border bg-slate-900 border-slate-800 text-slate-100 relative box-border">
      ...
      <div className="flex justify-between items-center mb-6 text-xs font-black tracking-wider text-slate-400">
        ...
        <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl bg-indigo-950/60 border border-indigo-500/30 text-indigo-400">
          <Sparkles className="w-3.5 h-3.5" /> Word Search
        </span>
      ...
      <div className="mb-6 p-4 bg-indigo-950/40 rounded-2xl border border-indigo-500/30 text-center">
        <span className="text-xs font-bold text-indigo-400 uppercase tracking-wide block mb-1">Petunjuk Utama</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {/* Sisi Kiri: Papan Grid & Tombol Aksi Verifikasi */}
                ...
                row.map((char, cIdx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-7 h-7 md:w-8 md:h-8 rounded-xl font-black text-xs md:text-sm flex items-center justify-center transition-all cursor-pointer ${
                        isSelected
        ...
        {/* Sisi Kanan: Panel Target Kata & Hint Interaktif */}
        <div className="lg:col-span-5 bg-slate-950/60 p-5 rounded-3xl border border-slate-800 space-y-3 w-full">
                  ...
                  {isHintOpen && hasHint && (
                    <div className="mt-2.5 p-2.5 bg-amber-950/40 border border-amber-500/30 rounded-xl text-xs text-amber-200 font-medium animate-in fade-in duration-150 flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        <span className="font-bold block text-[10px] uppercase text-amber-400">Petunjuk Kata:</span>
                        {item.hint}
                      </div>
                    </div>
                  )}
          ...
          <div className="pt-1 text-[11px] text-slate-500 text-center font-medium">
            Pilih huruf secara berurutan di papan, lalu klik "Cek Kata".
          </div>
        </div>
