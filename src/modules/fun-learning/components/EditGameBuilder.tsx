// src/modules/fun-learning/components/EditGameBuilder.tsx
'use client';
...
import { useEditGameBuilderState } from '../hooks/useEditGameBuilderState';
...
interface EditGameBuilderProps {
  tenantId: string;
  gameId: string;
  onBack: () => void;
  onSuccess?: () => void;
...
export default function EditGameBuilder({ tenantId, gameId, onBack, onSuccess }: EditGameBuilderProps) {
  const {
    title, setTitle,
    subject, setSubject,
  ...
  } = useEditGameBuilderState(tenantId, gameId);
  ...
  const handleUpdate = async (e: React.FormEvent, isPublished: boolean) => {
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: isPublished ? 'Modul game berhasil diperbarui dan dipublikasikan! 🚀' : 'Draft perubahan game berhasil disimpan.'
      });
      if (onSuccess) onSuccess();
    } catch (err: any) {
      setMessage({ type: 'error', text: `Gagal memperbarui game: ${err.message}` });
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-12 text-center text-sm text-slate-400 animate-pulse">
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-6 md:p-8">
      <div className="mb-6 border-b pb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-800">Edit Modul Game Pembelajaran</h2>
          <p className="text-xs text-slate-400 mt-0.5">Perbarui tahapan misi, blok latihan, atau target kelas.</p>
        </div>
        <button
          type="button"
          onClick={onBack}
      ...
      <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3 pt-6 border-t">
          ...
          <button type="button" disabled={saving} onClick={(e) => handleUpdate(e, false)} className="px-6 py-3.5 border border-slate-300 hover:bg-slate-50 rounded-2xl text-sm font-bold text-slate-700 transition cursor-pointer">
            Simpan Draft Perubahan
          </button>
          <button type="button" disabled={saving} onClick={(e) => handleUpdate(e, true)} className="px-6 py-3.5 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl text-sm font-bold shadow-lg shadow-blue-500/25 transition cursor-pointer">
            {saving ? 'Menyimpan...' : 'Perbarui & Publish 🚀'}
          </button>
        </div>
      </form>
